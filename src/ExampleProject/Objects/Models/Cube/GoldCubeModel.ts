...
import { BlankCubeTexture } from './Textures/BlankCubeTexture';

export class GoldCubeModel extends Model<TextureShader> {
    model_identifier: string = 'gold-cube';
    shader: TextureShader = GoldCubeModel.ShaderProvider.getShader(new TextureShader());
    mesh: CubeMesh = GoldCubeModel.MeshProvider.getMesh(new CubeMesh());
    textures: TextureMap[] = [
        new BlankCubeTexture()
    ];
    material: GoldMaterial = new GoldMaterial();
